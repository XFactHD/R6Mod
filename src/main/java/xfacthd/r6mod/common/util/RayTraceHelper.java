...
public class RayTraceHelper
{
    ...
    public static List<HitData> raytraceGunShot(World world, PlayerEntity shooter, int range, int maxPenetration, float spread)
    {
        ...
        Vec3d dirVecNormal = shooter.getLookVec().normalize().add(spreadX, spreadY, spreadZ);
        Vec3d startVec = shooter.getPositionVec().add(0, shooter.getEyeHeight(), 0);
        Vec3d endVec = startVec.add(dirVecNormal.scale(range));

        BlockRayTraceResult result = raytraceRangeDetailed(world, shooter, startVec, endVec);
        Vec3d rangeVec = result.getHitVec();
    ...
    public static boolean raytraceClaymore(TileEntityClaymore te, Vec3d start, Vec3d left, Vec3d middle, Vec3d right)
    {
    ...
    public static List<HitData> raytraceSonicBurst(EntityYokaiDrone drone, int range)
    {
        ...
        Vec3d dirVecNormal = drone.getLookVec().normalize();
        Vec3d startVec = drone.getPositionVec().add(0, drone.getEyeHeight(), 0).add(dirVecNormal);
        Vec3d endVec = startVec.add(dirVecNormal.scale(range));

        Vec3d rangeVec = raytraceRange(drone.world, shooter, startVec, endVec);
    ...
    public static HitData raytraceEvilEyeLaser(EntityEvilEyeCamera camera, int range)
    {
        ...
        Vec3d dirVecNormal = camera.getLookVec().normalize();
        Vec3d startVec = camera.getPositionVec().add(0, camera.getEyeHeight(), 0).add(dirVecNormal);
        Vec3d endVec = startVec.add(dirVecNormal.scale(range));
    ...
    public static Vec3d raytraceRange(World world, PlayerEntity player, Vec3d start, Vec3d end)
    {
        if (player == null) { return end; }
        return raytraceRangeDetailed(world, player, start, end).getHitVec();
    }

    public static BlockRayTraceResult raytraceRangeDetailed(World world, @Nonnull PlayerEntity player, Vec3d start, Vec3d end)
    {
    ...
    public static List<HitData> raytraceEntities(World world, PlayerEntity shooter, Vec3d start, Vec3d end, int maxCount, boolean ignoreShooter, Predicate<? super Entity> predicate)
    {
        return raytraceEntities(world, shooter, shooter.getPositionVec(), start, end, maxCount, ignoreShooter, predicate);
    }

    public static List<HitData> raytraceEntities(World world, @Nullable PlayerEntity shooter, Vec3d sourceVec, Vec3d start, Vec3d end, int maxCount, boolean ignoreShooter, Predicate<? super Entity> predicate)
    {
        ...
        for (Entity entity : entities)
        {
            if (!entity.noClip)
            {
                Optional<Vec3d> intercept = entity.getBoundingBox().rayTrace(start, end);
    ...
    private static boolean isHeadshot(Vec3d hitVec, LivingEntity entityHit)
    {
        double hit = hitVec.y;
