...
public class ItemCandela extends Item
{
    ...
    public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean selected)
    {
        if (stack.hasTag())
        {
            ...
            if (lastInteract != 0 && world.getGameTime() - lastInteract > 5)
            {
                ...
                if (selected && entity instanceof PlayerEntity && !world.isRemote())
                {
                    ...
                    if (!player.isCreative())
                    {
                        ...
                        {
                            //noinspection ConstantConditions
                            helmet.getTag().putBoolean("active", true);
                        }
    ...
    public ActionResultType onItemUseFirst(ItemStack stack, ItemUseContext context)
    {
        ...
        return handleRightClick(context.getWorld(), context.getItem(), context.getHand(), context.getPos(), context.getFace()).getType();
    }

    @Override
    public ActionResult<ItemStack> onItemRightClick(World world, PlayerEntity player, Hand hand)
    {
        return handleRightClick(world, player.getHeldItem(hand), hand, null, null);
    }

    private ActionResult<ItemStack> handleRightClick(World world, ItemStack stack, Hand hand, BlockPos target, Direction side)
    {
        if (hand == Hand.MAIN_HAND)
        {
            ...
            //noinspection ConstantConditions
            long stored = stack.getTag().getLong("pos");
            if (stored != 0 && target == null)
            {
                stack.getTag().putLong("pos", 0);
                stack.getTag().putInt("side", -1);
            }
            else if (target != null && (stored == 0 || stored != target.toLong()))
            {
                stack.getTag().putLong("pos", target.toLong());
                stack.getTag().putInt("side", side.getIndex());
            }
