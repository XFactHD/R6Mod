...
public class ItemMagazine extends Item
{
    ...
    {
        tooltip.add(new TranslationTextComponent("info.r6mod.mag_info").appendText(" " + getDamage(stack) + "/" + getMaxDamage(stack)));
        String caliberName = mag.getBullet() == null ? "Unknown" : mag.getBullet().toCaliberName();
        tooltip.add(new TranslationTextComponent("info.r6mod.caliber").appendText(" " + caliberName));
    }
