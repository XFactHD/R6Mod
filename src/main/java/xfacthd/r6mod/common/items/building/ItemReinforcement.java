...
public class ItemReinforcement extends Item implements IPlacementTime
{
    ...
    public ActionResultType onItemUse(ItemUseContext context)
    {
        ...
        if (!Config.INSTANCE.usageTime || placeTime >= getPlacementTime())
        {
    ...
    private ActionResultType tryPlaceWallReinforcement(BlockItemUseContext context)
    {
        ...
        Vector3d soundPos = getFrontCenteredSound(base.getA(), facing);
    ...
    private static Tuple<BlockPos, BlockPos> getBasePositions(BlockPos pos, Direction facing, Vector3d subHit)
    {
    ...
    private static Vector3d getSubHit(Vector3d hitVec)
    {
        return hitVec.subtract((int)hitVec.getX(), (int)hitVec.getY(), (int)hitVec.getZ());
    }

    private static boolean isLeft(Vector3d subHit, Direction facing)
    {
        ...
        else
        {
            ...
            return facing.getAxisDirection() == Direction.AxisDirection.NEGATIVE ? x < .5 : x > .5;
        }
    }

    private static Direction getOffset(Direction facing, Vector3d subHit)
    {
