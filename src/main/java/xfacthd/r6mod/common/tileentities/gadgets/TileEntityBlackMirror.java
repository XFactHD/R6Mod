...
public class TileEntityBlackMirror extends TileEntityGadget
{
        ...
        if (!world.isRemote() && waitingGlassFall)
        {
            if (world.getGameTime() - destroyTime > GLASS_DELAY)
            {
                ...
                if (!getBlockState().get(PropertyHolder.RIGHT)) //Only play the sound once
                {
                    ...
                    Vec3d soundPos = getFrontCenteredSound(pos, facing);
    ...
    public boolean destroyCanister(Vec3d hitVec)
    {
        ...
        if (!state.get(PropertyHolder.DESTROYED))
        {
            ...
            Vec3d subHit = hitVec.subtract((int)hitVec.getX(), pos.getY(), (int)hitVec.getZ());
            ...
            if (isCanisterHit(facing, right, subHit))
            {
                ...
                if (!world.isRemote())
                {
                    ...
                    Vec3d soundPos = getFrontCenteredSound(!right ? pos : pos.offset(facing.rotateYCCW()), facing);
    ...
    private static boolean isCanisterHit(Direction facing, boolean right, Vec3d subHit)
    {
        ...
        {
            return z >= .89D && (right ? x >= 0F && x <= 3F/16F : x >= 13F/16F && x <= 1F);
        }
