...
public class TileEntityBreachCharge extends TileEntityGadget implements IActivatable, IExplosionParticleSpawner
{
    ...
    private static final HashMap<Direction, Vector3d> ORIGIN_VECS = new HashMap<>();

    static
    {
        ORIGIN_VECS.put(Direction.NORTH, new Vector3d(0.5, 0.5,   0));
        ORIGIN_VECS.put(Direction.EAST,  new Vector3d(  1, 0.5, 0.5));
        ORIGIN_VECS.put(Direction.SOUTH, new Vector3d(0.5, 0.5,   1));
        ORIGIN_VECS.put(Direction.WEST,  new Vector3d(  0, 0.5, 0.5));
        ORIGIN_VECS.put(Direction.DOWN,  new Vector3d(0.5,   0, 0.5));
    }
    ...
    private void damageEntities()
    {
        ...
        Vector3d origin = Vector3d.copy(pos).add(ORIGIN_VECS.get(facing));
        List<Entity> entities = world.getEntitiesInAABBexcluding(null, frontBB, DAMAGE_PREDICATE);
        for (Entity entity : entities)
        {
            Vector3d entityVec = entity.getPositionVec().add(0, entity.getHeight() / 2D, 0);
