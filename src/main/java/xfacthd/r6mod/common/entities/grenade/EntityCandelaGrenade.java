...
public class EntityCandelaGrenade extends AbstractEntityGrenade implements IEntityAdditionalSpawnData, IShootable
{
    ...
    private static final Vec3d VEC_UP = new Vec3d(0, 1, 0);


    private boolean onBlock = false;
    private Vec3d spawnOffset = null;
        ...
        if (thrower.isSneaking())
        {
            ...
            if (result.getType() == RayTraceResult.Type.BLOCK)
            {
                ...
                if (onBlock)
                {
                    ...
                    Vec3d dir = new Vec3d(side.getOpposite().getDirectionVec());
                    BlockRayTraceResult oppResult = RayTraceHelper.raytraceRangeDetailed(world, thrower, hit.add(dir), hit); //Reverse raytrace from opposite side
    ...
    private void tickFlashbangSpawns()
    {
        ...
        if (!world.isRemote() && world.getGameTime() - lastFlashbang >= FLASHBANG_INTERVAL)
        {
            ...
            Vec3d motion; //TODO: calculate deviation from straight vector
    ...
    protected void readAdditional(CompoundNBT nbt)
    {
        ...
        if (onBlock)
        {
            ...
            spawnOffset = new Vec3d(offX, offY, offZ);
        }
    }
