...
public class EffectYokaiBlast extends AbstractEffect
{
    ...
    public void drawEffect(MatrixStack matrix)
    {
        ...
        bufferbuilder.pos(matrix.getLast().getMatrix(), 0, h, 0).tex(sprite.getMinU(), sprite.getMaxV()).color(0xFF, 0xFF, 0xFF, alpha).endVertex();
        bufferbuilder.pos(matrix.getLast().getMatrix(), w, h, 0).tex(sprite.getMaxU(), sprite.getMaxV()).color(0xFF, 0xFF, 0xFF, alpha).endVertex();
        bufferbuilder.pos(matrix.getLast().getMatrix(), w, 0, 0).tex(sprite.getMaxU(), sprite.getMinV()).color(0xFF, 0xFF, 0xFF, alpha).endVertex();
        bufferbuilder.pos(matrix.getLast().getMatrix(), 0, 0, 0).tex(sprite.getMinU(), sprite.getMinV()).color(0xFF, 0xFF, 0xFF, alpha).endVertex();
