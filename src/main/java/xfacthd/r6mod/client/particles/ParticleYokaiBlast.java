...
public class ParticleYokaiBlast extends Particle
{
    ...
    public ParticleYokaiBlast(World world, double posX, double posY, double posZ, float yaw, float pitch)
    {
    ...
    public void tick()
    {
        ...
        else
        {
            if (motionX != 0.0D || motionY != 0.0D || motionZ != 0.0D)
            {
                ...
                Vec3d actual = Entity.collideBoundingBoxHeuristically(null, motion, getBoundingBox(), world, ISelectionContext.dummy(), new ReuseableStream<>(Stream.empty()));
    ...
    public static class Factory implements IParticleFactory<ParticleDataYokaiBlast>
    {
        @Nullable
        @Override
        public Particle makeParticle(ParticleDataYokaiBlast data, World world, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed)
        {
