...
public class RenderClaymore extends TileEntityRenderer<TileEntityClaymore>
{
    ...
    public void render(TileEntityClaymore te, float partialTicks, MatrixStack matrix, IRenderTypeBuffer buffer, int combinedLight, int combinedOverlay)
    {
        if (te.isActive())
        {
            Vec3d posVec = ORIGIN_VECTORS.get(te.getFacing());
            Triple<Vec3d, Vec3d, Vec3d> laserVecs = te.getLasers();
            ...
            Vec3d player = Minecraft.getInstance().player.getPositionVec();
            double distance = te.getDistanceSq(player.x, player.y, player.z);
