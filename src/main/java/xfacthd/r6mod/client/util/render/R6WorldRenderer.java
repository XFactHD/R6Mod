...
public class R6WorldRenderer extends WorldRenderer
{
    ...
    private final ObjectList<LocalRenderInformationContainer> renderInfos = new ObjectArrayList<>(69696);
    ...
    public R6WorldRenderer(Minecraft mcIn, RenderTypeBuffers rainTimeBuffersIn) {
    ...
    private void setupTerrain(ActiveRenderInfo activeRenderInfoIn, ClippingHelperImpl camera, boolean debugCamera, int frameCount, boolean playerSpectator) {
        ...
        if (!debugCamera && this.displayListEntitiesDirty) {
            ...
            if (chunkrenderdispatcher$chunkrender != null) {
                boolean flag1 = false;
                LocalRenderInformationContainer worldrenderer$localrenderinformationcontainer3 = new LocalRenderInformationContainer(chunkrenderdispatcher$chunkrender, null, 0);
                ...
                list.sort(Comparator.comparingDouble((p_230016_1_) -> blockpos.distanceSq(p_230016_1_.renderChunk.getPosition().add(8, 8, 8))));
                queue.addAll(list);
            }
            ...
            while(!queue.isEmpty()) {
                ...
                for(Direction direction2 : FACINGS) {
                    ...
                    if ((!flag || !worldrenderer$localrenderinformationcontainer1.hasDirection(direction2.getOpposite())) && (!flag || direction == null || chunkrenderdispatcher$chunkrender3.getCompiledChunk().isVisible(direction.getOpposite(), direction2)) && chunkrenderdispatcher$chunkrender2 != null && chunkrenderdispatcher$chunkrender2.shouldStayLoaded() && chunkrenderdispatcher$chunkrender2.setFrameIndex(frameCount) && camera.isBoundingBoxInFrustum(chunkrenderdispatcher$chunkrender2.boundingBox)) {
                        LocalRenderInformationContainer worldrenderer$localrenderinformationcontainer = new LocalRenderInformationContainer(chunkrenderdispatcher$chunkrender2, direction2, worldrenderer$localrenderinformationcontainer1.counter + 1);
    ...
    public void updateCameraAndRender(MatrixStack matrixStackIn, float partialTicks, long finishTimeNano, boolean drawBlockOutline, ActiveRenderInfo renderInfo, GameRenderer gameRendererIn, LightTexture lightmapIn, Matrix4f projectionIn) {
        TileEntityRendererDispatcher.instance.prepare(this.world, this.mc.getTextureManager(), this.mc.fontRenderer, renderInfo, this.mc.objectMouseOver);
        this.renderManager.cacheActiveRenderInfo(this.world, renderInfo, this.mc.pointedEntity);
        ...
        if (flag) {
            ...
            clippingHelper = new ClippingHelperImpl(matrix4f, projectionIn);
            clippingHelper.setCameraPosition(x, y, z);
        }

        this.mc.getProfiler().endStartSection("captureFrustum");
        if (this.debugFixTerrainFrustum) {
            this.captureFrustum(matrix4f, projectionIn, vec3d.x, vec3d.y, vec3d.z, flag ? new ClippingHelperImpl(matrix4f, projectionIn) : clippingHelper);
            this.debugFixTerrainFrustum = false;
        }

        iprofiler.endStartSection("clear");
        FogRenderer.updateFogColor(renderInfo, partialTicks, this.mc.world, this.mc.gameSettings.renderDistanceChunks, gameRendererIn.getBossColorModifier(partialTicks));
        RenderSystem.clear(16640, Minecraft.IS_RUNNING_ON_MAC);
        float f = gameRendererIn.getFarPlaneDistance();
        boolean flag1 = this.mc.world.dimension.doesXZShowFog(MathHelper.floor(x), MathHelper.floor(y)) || this.mc.ingameGUI.getBossOverlay().shouldCreateFog();
        if (this.mc.gameSettings.renderDistanceChunks >= 4) {
            FogRenderer.setupFog(renderInfo, FogRenderer.FogType.FOG_SKY, f, flag1, partialTicks);
            iprofiler.endStartSection("sky");
            this.renderSky(matrixStackIn, partialTicks);
        }

        iprofiler.endStartSection("fog");
        FogRenderer.setupFog(renderInfo, FogRenderer.FogType.FOG_TERRAIN, Math.max(f - 16.0F, 32.0F), flag1, partialTicks);
        iprofiler.endStartSection("terrain_setup");
        this.setupTerrain(renderInfo, clippingHelper, flag, this.frameId++, this.mc.player.isSpectator());
        ...
        this.renderBlockLayer(RenderType.getSolid(), matrixStackIn, x, y, z);
        ...
        this.renderBlockLayer(RenderType.getCutoutMipped(), matrixStackIn, x, y, z);
        this.mc.getModelManager().getAtlasTexture(AtlasTexture.LOCATION_BLOCKS_TEXTURE).restoreLastBlurMipmap();
        this.renderBlockLayer(RenderType.getCutout(), matrixStackIn, x, y, z);
        ...
        for(Entity entity : this.world.getAllEntities()) {
            if (
                    (renderManager.shouldRender(entity, clippingHelper, x, y, z) || entity.isRidingOrBeingRiddenBy(mc.player)) &&
                    (entity != renderInfo.getRenderViewEntity() || renderInfo.isThirdPerson() || renderInfo.getRenderViewEntity() instanceof LivingEntity && ((LivingEntity)renderInfo.getRenderViewEntity()).isSleeping()) &&
                    (!(entity instanceof ClientPlayerEntity) || renderInfo.getRenderViewEntity() == entity || !mc.player.isSpectator())) {
                ...
                this.renderEntity(entity, x, y, z, partialTicks, matrixStackIn, irendertypebuffer);
            }
        }
        ...
        for(LocalRenderInformationContainer worldrenderer$localrenderinformationcontainer : this.renderInfos) {
            ...
            if (!list.isEmpty()) {
                for(TileEntity tileentity1 : list) {
                    if(!clippingHelper.isBoundingBoxInFrustum(tileentity1.getRenderBoundingBox())) continue;
                    BlockPos blockpos3 = tileentity1.getPos();
                    IRenderTypeBuffer irendertypebuffer1 = irendertypebuffer$impl;
                    matrixStackIn.push();
                    matrixStackIn.translate((double)blockpos3.getX() - x, (double)blockpos3.getY() - y, (double)blockpos3.getZ() - z);
        ...
        synchronized(this.setTileEntities) {
            for(TileEntity tileentity : this.setTileEntities) {
                if(!clippingHelper.isBoundingBoxInFrustum(tileentity.getRenderBoundingBox())) continue;
                BlockPos blockpos2 = tileentity.getPos();
                matrixStackIn.push();
                matrixStackIn.translate((double)blockpos2.getX() - x, (double)blockpos2.getY() - y, (double)blockpos2.getZ() - z);
        ...
        for(Long2ObjectMap.Entry<SortedSet<DestroyBlockProgress>> entry : this.damageProgress.long2ObjectEntrySet()) {
            ...
            if (!(d3 * d3 + d4 * d4 + d5 * d5 > 1024.0D)) {
                ...
                if (sortedset1 != null && !sortedset1.isEmpty()) {
                    ...
                    matrixStackIn.push();
                    matrixStackIn.translate((double)blockpos1.getX() - x, (double)blockpos1.getY() - y, (double)blockpos1.getZ() - z);
        ...
        if (drawBlockOutline && raytraceresult != null && raytraceresult.getType() == RayTraceResult.Type.BLOCK) {
            ...
            if (!net.minecraftforge.client.ForgeHooksClient.onDrawBlockHighlight(this, renderInfo, mc.objectMouseOver, partialTicks, matrixStackIn, irendertypebuffer$impl))
                if (!blockstate.isAir(this.world, blockpos) && this.world.getWorldBorder().contains(blockpos)) {
                    ...
                    this.drawSelectionBox(matrixStackIn, ivertexbuilder2, renderInfo.getRenderViewEntity(), x, y, z, blockpos, blockstate);
                }
        }

        RenderSystem.pushMatrix();
        RenderSystem.multMatrix(matrixStackIn.getLast().getMatrix());
        this.mc.debugRenderer.render(matrixStackIn, irendertypebuffer$impl, x, y, z);
        this.renderWorldBorder(renderInfo);
        ...
        this.renderBlockLayer(RenderType.getTranslucent(), matrixStackIn, x, y, z);
        iprofiler.endStartSection("particles");
        this.mc.particles.renderParticles(matrixStackIn, irendertypebuffer$impl, lightmapIn, renderInfo, partialTicks);
        RenderSystem.pushMatrix();
        RenderSystem.multMatrix(matrixStackIn.getLast().getMatrix());
        ...
        if (this.mc.gameSettings.getCloudOption() != CloudOption.OFF) {
            ...
            this.renderClouds(matrixStackIn, partialTicks, x, y, z);
        }

        RenderSystem.depthMask(false);
        iprofiler.endStartSection("weather");
        this.renderRainSnow(lightmapIn, partialTicks, x, y, z);
        RenderSystem.depthMask(true);
        this.renderDebug(renderInfo);
    ...
    private void renderBlockLayer(RenderType blockLayerIn, MatrixStack matrixStackIn, double xIn, double yIn, double zIn) {
        ...
        this.mc.getProfiler().endStartSection(() -> "render_" + blockLayerIn);
        boolean flag = blockLayerIn != RenderType.getTranslucent();
        ObjectListIterator<LocalRenderInformationContainer> objectlistiterator = this.renderInfos.listIterator(flag ? 0 : this.renderInfos.size());
            ...
            for(LocalRenderInformationContainer worldrenderer$localrenderinformationcontainer : this.renderInfos) {
                ...
                if (this.mc.debugWireframe) {
                    ...
                    if (direction != null) {
                        ...
                        bufferbuilder.pos(8 - 16 * direction.getXOffset(), 8 - 16 * direction.getYOffset(), 8 - 16 * direction.getZOffset()).color(j, k, l, 255).endVertex();
                    }
                    ...
                    for(Direction direction2 : Direction.values()) {
                        for(Direction direction1 : Direction.values()) {
                            ...
                            if (!flag) {
                                ...
                                bufferbuilder.pos(8 + 8 * direction1.getXOffset(), 8 + 8 * direction1.getYOffset(), 8 + 8 * direction1.getZOffset()).color(1, 0, 0, 1).endVertex();
                            }
                        }
                    }
    ...
    private void addDebugVertex(IVertexBuilder bufferIn, int vertex) {
        bufferIn.pos(this.debugTerrainMatrix[vertex].getX(), this.debugTerrainMatrix[vertex].getY(), this.debugTerrainMatrix[vertex].getZ()).endVertex();
    }
    ...
    private void addDebugQuad(IVertexBuilder bufferIn, int vertex1, int vertex2, int vertex3, int vertex4, int red, int green, int blue) {
        bufferIn.pos(this.debugTerrainMatrix[vertex1].getX(), this.debugTerrainMatrix[vertex1].getY(), this.debugTerrainMatrix[vertex1].getZ()).color((float)red, (float)green, (float)blue, 0.25F).endVertex();
        bufferIn.pos(this.debugTerrainMatrix[vertex2].getX(), this.debugTerrainMatrix[vertex2].getY(), this.debugTerrainMatrix[vertex2].getZ()).color((float)red, (float)green, (float)blue, 0.25F).endVertex();
        bufferIn.pos(this.debugTerrainMatrix[vertex3].getX(), this.debugTerrainMatrix[vertex3].getY(), this.debugTerrainMatrix[vertex3].getZ()).color((float)red, (float)green, (float)blue, 0.25F).endVertex();
        bufferIn.pos(this.debugTerrainMatrix[vertex4].getX(), this.debugTerrainMatrix[vertex4].getY(), this.debugTerrainMatrix[vertex4].getZ()).color((float)red, (float)green, (float)blue, 0.25F).endVertex();
    }
    ...
    public void renderSky(MatrixStack matrixStackIn, float partialTicks) {
        ...
        if (renderer != null) {
            if (renderer instanceof net.minecraftforge.client.SkyRenderHandler) {
                ((net.minecraftforge.client.SkyRenderHandler) renderer).render(this.ticks, partialTicks, matrixStackIn, this.world, this.mc);
            } else {
    ...
    public void renderClouds(MatrixStack matrixStackIn, float partialTicks, double viewEntityX, double viewEntityY, double viewEntityZ) {
        ...
        if (renderer != null) {
            if (renderer instanceof net.minecraftforge.client.CloudRenderHandler) {
                ((net.minecraftforge.client.CloudRenderHandler) renderer).render(this.ticks, partialTicks, matrixStackIn, this.world, this.mc);
            } else {
                renderer.render(this.ticks, partialTicks, this.world, this.mc);
            }
            return;
        ...
        if (this.mc.world.dimension.isSurfaceWorld()) {
            ...
            RenderSystem.defaultBlendFunc();
            RenderSystem.enableFog();
            double d1 = ((float)this.ticks + partialTicks) * 0.03F;
            double d2 = (viewEntityX + d1) / 12.0D;
            double d3 = this.world.dimension.getCloudHeight() - (float)viewEntityY + 0.33F;
            ...
            matrixStackIn.translate(-f2, f3, -f4);
            if (this.cloudsVBO != null) {
        ...
        switch(type) {
            ...
            case 1010:
                if (Item.getItemById(data) instanceof MusicDiscItem) {
                    ...
                    this.playRecord(null, blockPosIn);
                }
                break;
            case 1011:
            ...
            case 1503:
                ...
                for(int i1 = 0; i1 < 16; ++i1) {
                    double d15 = (float)blockPosIn.getX() + (5.0F + random.nextFloat() * 6.0F) / 16.0F;
                    double d21 = (float)blockPosIn.getY() + 0.8125F;
                    double d26 = (float)blockPosIn.getZ() + (5.0F + random.nextFloat() * 6.0F) / 16.0F;
                    double d29 = 0.0D;
                    double d30 = 0.0D;
                    double d5 = 0.0D;
            ...
            case 2007:
                ...
                float f5 = (float)(data & 255) / 255.0F;
            ...
            case 2006:
                for(int j = 0; j < 200; ++j) {
                    ...
                    double d1 = MathHelper.cos(f1) * f;
                    double d2 = 0.01D + random.nextDouble() * 0.5D;
                    double d3 = MathHelper.sin(f1) * f;
    ...
    public void sendBlockBreakProgress(int breakerId, BlockPos pos, int progress) {
        if (progress >= 0 && progress < 10) {
            ...
            this.damageProgress.computeIfAbsent(destroyblockprogress1.getPosition().toLong(), (p_230017_0_) -> Sets.newTreeSet()).add(destroyblockprogress1);
        } else {
    ...
    @OnlyIn(Dist.CLIENT)
    static class LocalRenderInformationContainer {
