...
public class R6WorldRenderer extends WorldRenderer implements IResourceManagerReloadListener, AutoCloseable {
    ...
    private void setupTerrain(ActiveRenderInfo activeRenderInfoIn, ClippingHelper camera, boolean debugCamera, int frameCount, boolean playerSpectator) {
        ...
        if (!debugCamera && this.displayListEntitiesDirty) {
            ...
            Entity.setRenderDistanceWeight(MathHelper.clamp((double)this.mc.gameSettings.renderDistanceChunks / 8.0D, 1.0D, 2.5D) * (double)this.mc.gameSettings.field_238329_c_);
            ...
            while(!queue.isEmpty()) {
                ...
                for(Direction direction1 : FACINGS) {
                    ...
                    if ((!flag || !worldrenderer$localrenderinformationcontainer1.hasDirection(direction1.getOpposite())) && (!flag || direction == null || chunkrenderdispatcher$chunkrender3.getCompiledChunk().isVisible(direction.getOpposite(), direction1)) && chunkrenderdispatcher$chunkrender2 != null && chunkrenderdispatcher$chunkrender2.shouldStayLoaded() && chunkrenderdispatcher$chunkrender2.setFrameIndex(frameCount) && camera.isBoundingBoxInFrustum(chunkrenderdispatcher$chunkrender2.boundingBox)) {
                        LocalRenderInformationContainer worldrenderer$localrenderinformationcontainer = new LocalRenderInformationContainer(chunkrenderdispatcher$chunkrender2, direction1, worldrenderer$localrenderinformationcontainer1.counter + 1);
        ...
        for(LocalRenderInformationContainer worldrenderer$localrenderinformationcontainer2 : this.renderInfos) {
            ...
            if (chunkrenderdispatcher$chunkrender4.needsUpdate() || set.contains(chunkrenderdispatcher$chunkrender4)) {
                ...
                if (net.minecraftforge.common.ForgeConfig.CLIENT.alwaysSetupTerrainOffThread.get() || !chunkrenderdispatcher$chunkrender4.needsImmediateUpdate() && !flag1) {
    ...
    public void updateCameraAndRender(MatrixStack matrixStackIn, float partialTicks, long finishTimeNano, boolean drawBlockOutline, ActiveRenderInfo activeRenderInfoIn, GameRenderer gameRendererIn, LightTexture lightmapIn, Matrix4f projectionIn) {
        ...
        for(Entity entity : this.world.getAllEntities()) {
            if ((this.renderManager.shouldRender(entity, clippinghelper, d0, d1, d2) || entity.isRidingOrBeingRiddenBy(this.mc.player)) && (entity != activeRenderInfoIn.getRenderViewEntity() || activeRenderInfoIn.isThirdPerson() || activeRenderInfoIn.getRenderViewEntity() instanceof LivingEntity && ((LivingEntity)activeRenderInfoIn.getRenderViewEntity()).isSleeping()) && (!(entity instanceof ClientPlayerEntity) || activeRenderInfoIn.getRenderViewEntity() == entity || !mc.player.isSpectator())) {
                ...
                if (this.isRenderEntityOutlines() && this.mc.func_238206_b_(entity)) {
                    flag2 = true;
    ...
    public void playRecord(@Nullable SoundEvent soundIn, BlockPos pos, @Nullable MusicDiscItem musicDiscItem) {
        ...
        if (soundIn != null) {
            if (musicDiscItem != null) {
                this.mc.ingameGUI.func_238451_a_(musicDiscItem.func_234801_g_());
            }

            ISound simplesound = SimpleSound.record(soundIn, pos.getX(), pos.getY(), pos.getZ());
    ...
    public void playEvent(PlayerEntity player, int type, BlockPos blockPosIn, int data) {
        ...
        switch(type) {
            ...
            case 1031:
                ...
                break;
            case 1032:
                this.mc.getSoundHandler().play(SimpleSound.func_239532_b_(SoundEvents.BLOCK_PORTAL_TRAVEL, random.nextFloat() * 0.4F + 0.8F, 0.25F));
                break;
            case 1033:
