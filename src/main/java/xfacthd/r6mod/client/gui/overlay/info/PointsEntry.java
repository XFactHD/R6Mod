...
public class PointsEntry
{
    ...
    public int draw(MatrixStack matrix, int xRight, int yTop)
    {
        ...
        //Draw texture
        TextureDrawer.drawTexture(matrix, xLeft, yTop, ENTRY_WIDTH, BASE_HEIGHT, 0F, 1F, 0F, .75F);

        //Draw main points
        String text = (points >= 0 ? "+" : "") + points;
        int textX = xRight - font().getStringWidth(text) - TEXT_OFFSET;
        int color = points >= 0 ? 0xFFFFF500 : 0xFFFF0000;
        font().drawString(matrix, text, textX, yTop + TEXT_OFFSET, color);
        ...
        font().drawString(matrix, text, textX, yTop + font().FONT_HEIGHT + TEXT_OFFSET, color);
        ...
        if (extraPoints != null)
        {
            for (ExtraPointsEntry entry : extraPoints)
            {
                ...
                textX = xRight - font().getStringWidth(text) - TEXT_OFFSET;
                font().drawString(matrix, text, textX, yTop, 0xFFFFFFFF);
                ...
                font().drawString(matrix, text, textX, yTop, 0xFFBBBBBB);
