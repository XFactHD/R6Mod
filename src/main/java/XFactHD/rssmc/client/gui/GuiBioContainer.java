...
import XFactHD.rssmc.common.gui.ContainerBioContainer;
import XFactHD.rssmc.common.net.PacketSetBioContainerRange;
...
public class GuiBioContainer extends GuiContainerBase
{
    private static final String TEXTURE_MINUS = "gui/widgets/minus";
    private static final String TEXTURE_PLUS = "gui/widgets/plus";
    ...
    private String upLocalized = I18n.format("gui.rssmc:up.name");
    ...
    public GuiBioContainer(BlockPos pos)
    {
        super(new ContainerBioContainer());
        setGuiSize(130, 200);
        this.pos = pos;
    }
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY)
    {
        ...
        drawCenteredString(fontRendererObj, upLocalized    + ": " + Integer.toString(rangeUp),    guiLeft + 65, guiTop + 141, 16777215);
    }
    ...
    public void updateScreen()
    {
        ...
        buttonList.get( 8).enabled = rangeUp > 0;
        buttonList.get( 9).enabled = rangeUp < 8;
        buttonList.get(10).enabled = (rangeEast + rangeWest) * (rangeNorth + rangeSouth) > 0 && rangeUp > 0;
    }
    ...
    protected void actionPerformed(GuiButton button) throws IOException
    {
        ...
        if (button.id == 8) { rangeUp    -= 1; }
        if (button.id == 9) { rangeUp    += 1; }
        if (button.id == 10)
        {
            RainbowSixSiegeMC.NET.sendMessageToServer(new PacketSetBioContainerRange(pos, rangeNorth, rangeEast, rangeSouth, rangeWest, rangeUp));
    ...
    {
        super.onGuiClosed();
        if ((rangeEast + rangeWest) * (rangeNorth + rangeSouth) == 0 || rangeUp == 0)
        {
            RainbowSixSiegeMC.NET.sendMessageToServer(new PacketSetBioContainerRange(pos, 0, 0, 0, 0, 0));
            Minecraft.getMinecraft().player.sendMessage(new TextComponentTranslation("chat.rssmc:bio_cont_setup_cancled.name"));
        }
    }
}